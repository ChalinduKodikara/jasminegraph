...
############################################
logging.info("____________________________________ Client Started ____________________________________")
logging.info("____________________________________ Partition ID %s____________________________________", PARTITION_ID)
    ...
    def run(self):
        ...
        while not self.ITERATION_FLAG:
            ...
            if self.iteration_number <= self.NUM_TIMESTAMPS:
                ...
                while True:
                    try:
                        edges = pd.read_csv('models/files/' + self.dataset_name + '_' + str(PARTITION_SIZE) + '_' + str(PARTITION_ID) + '/' + str(self.iteration_number) + '_test_batch_edges.csv')
                        nodes = pd.read_csv('models/files/' + self.dataset_name + '_' + str(PARTITION_SIZE) + '_' + str(PARTITION_ID) + '/' + str(self.iteration_number) + '_test_batch_nodes.csv', index_col=0)
                        # edges.columns = ["source", 'target']
                        # edges['weight'] = 1
                        # nodes.columns = [str(i) for i in range(len(nodes.columns))]
                        break
                    except:
                        time.sleep(1)
...
if __name__ == "__main__":
    ...
    while True:
        try:
            edges = pd.read_csv('models/files/' + DATASET_NAME + '_' + str(PARTITION_SIZE) + '_' + str(PARTITION_ID) + '/' + str(0) + '_training_batch_edges.csv')
            nodes = pd.read_csv('models/files/' + DATASET_NAME + '_' + str(PARTITION_SIZE) + '_' + str(PARTITION_ID) + '/' + str(0) + '_training_batch_nodes.csv', index_col=0)
            # edges.columns = ["source", 'target']
            # edges['weight'] = 1
            # nodes.columns = [str(i) for i in range(len(nodes.columns))]
            break
