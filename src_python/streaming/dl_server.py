...
parser.add_argument('--num_of_timestamps', type=int, default=20, help='Number of timestamps')
...
NUM_TIMESTAMPS = args.num_of_timestamps
######## Our parameters ################
...
time.sleep(2)
############################################
logging.info("____________________________________ Server Started ____________________________________")
...
if __name__ == "__main__":
    ...
    while True:
        try:
            dummy = pd.read_csv('data/' + DATASET_NAME + '_' + str(PARTITION_SIZE) + '_' + str(PARTITION_ID) + '/' + str(0) + '_training_batch_edges.csv')
            dummy = pd.read_csv('data/' + DATASET_NAME + '_' + str(PARTITION_SIZE) + '_' + str(PARTITION_ID) + '/' + str(0) + '_training_batch_nodes.csv', index_col=0)
