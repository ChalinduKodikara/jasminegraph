...
void listen_to_kafka_topic_3(KafkaConnector *kstream, Partitioner &graphPartitioner, vector<DataPublisher *> &workerClients,
                             int noOfPartitions) {
...
//
            graphPartitioner.printStats();
            input_stream_handler = thread(listen_to_kafka_topic_3, kstream, std::ref(graphPartitioner),
                                          std::ref(workerClients), numberOfPartitions);

            JasmineGraphServer *jasmineServer = new JasmineGraphServer();
            jasmineServer->initiateStreamingDistributedLearning(sqlite);
