...
struct instanceservicesessionargs {
    ...
    std::map<std::string, JasmineGraphCsvStore *> csvStoreMap;
...
public:
    ...
    static JasmineGraphCsvStore* loadCsvStore(
            std::string graphId, std::string partitionId,
            std::map<std::string, JasmineGraphCsvStore*>& graphDBMapStreamingStores);
    ...
    static void initStreamingServer(string partitionID);

    static void initStreamingClient(string partitionID);
