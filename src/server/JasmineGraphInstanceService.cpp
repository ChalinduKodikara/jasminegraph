...
#include "../server/JasmineGraphServer.h"
#include "../localstore/incremental/JasmineGraphCsvStore.h"
#include <stdio.h>
...
void *instanceservicesession(void *dummyPt) {
    ...
    std::map<std::string, JasmineGraphCsvStore *> csvStoreMap = sessionargs->csvStoreMap;
    ...
    while (!loop) {
        ...
        if (line.compare(JasmineGraphInstanceProtocol::HANDSHAKE) == 0) {
        ...
        } else if (line.compare(JasmineGraphInstanceProtocol::INITIATE_STREAMING_SERVER) == 0) {

            instance_logger.log("Received : " + JasmineGraphInstanceProtocol::INITIATE_STREAMING_SERVER, "info");
            write(connFd, JasmineGraphInstanceProtocol::OK.c_str(), JasmineGraphInstanceProtocol::OK.size());
            instance_logger.log("Sent : " + JasmineGraphInstanceProtocol::OK, "info");
            bzero(data, INSTANCE_DATA_LENGTH);
            read(connFd, data, INSTANCE_DATA_LENGTH);
            ...
            string partitionID = trainData;

            std::thread workerThread = std::thread(&JasmineGraphInstanceService::initStreamingServer, partitionID);
            workerThread.join();

        } else if (line.compare(JasmineGraphInstanceProtocol::INITIATE_STREAMING_CLIENT) == 0) {

            instance_logger.log("Received : " + JasmineGraphInstanceProtocol::INITIATE_STREAMING_CLIENT, "info");
            write(connFd, JasmineGraphInstanceProtocol::OK.c_str(), JasmineGraphInstanceProtocol::OK.size());
            instance_logger.log("Sent : " + JasmineGraphInstanceProtocol::OK, "info");
            bzero(data, INSTANCE_DATA_LENGTH);
            read(connFd, data, INSTANCE_DATA_LENGTH);
            ...
            std::thread workerThread = std::thread(&JasmineGraphInstanceService::initStreamingClient, partitionID);
            workerThread.join();
        ...
        } else if (line.compare(JasmineGraphInstanceProtocol::GRAPH_CSV_STREAM_START) == 0) {

            write(connFd, JasmineGraphInstanceProtocol::GRAPH_CSV_STREAM_START_ACK.c_str(),
                  JasmineGraphInstanceProtocol::GRAPH_CSV_STREAM_START_ACK.size());
            // instance_logger.log("Sent : " + JasmineGraphInstanceProtocol::GRAPH_CSV_STREAM_START_ACK, "info");
            ...
            std::string nodeString(content_length, 0);
            send(connFd, JasmineGraphInstanceProtocol::GRAPH_CSV_STREAM_C_length_ACK.c_str(),
                 JasmineGraphInstanceProtocol::GRAPH_CSV_STREAM_C_length_ACK.size(), 0);
            // instance_logger.log("Acked for content length", "info");
            ...
            auto graphIdPartitionId = JasmineGraphCsvStore::getIDs(nodeString);
            std::string graphId = graphIdPartitionId.first;
            std::string partitionId = graphIdPartitionId.second;
            std::string graphIdentifier = graphId + "_" + partitionId;
            JasmineGraphCsvStore *csvStoreInstance;

            if (csvStoreMap.find(graphIdentifier) == csvStoreMap.end()) {
                csvStoreInstance =
                        JasmineGraphInstanceService::loadCsvStore(graphId, partitionId, csvStoreMap);
            } else {
                csvStoreInstance = csvStoreMap[graphIdentifier];
            }
            csvStoreInstance->saveCsvData(nodeString);

            send(connFd, JasmineGraphInstanceProtocol::GRAPH_CSV_STREAM_END_OF_EDGE.c_str(),
                 JasmineGraphInstanceProtocol::GRAPH_CSV_STREAM_END_OF_EDGE.size(), 0);
            // instance_logger.log("Sent CRLF string to mark the end", "info");
...
JasmineGraphCsvStore *JasmineGraphInstanceService::loadCsvStore(
        std::string graphId, std::string partitionId,
        std::map<std::string, JasmineGraphCsvStore *> &graphDBMapCsvStores) {
    std::string graphIdentifier = graphId + "_" + partitionId;
    instance_logger.log("###INSTANCE### Loading Csv Store for" + graphIdentifier + " : Started", "info");
    Utils utils;
//    std::string folderLocation = utils.getJasmineGraphProperty("org.jasminegraph.server.instance.datafolder");
    JasmineGraphCsvStore *jasmineGraphCsvStore =
            new JasmineGraphCsvStore(atoi(graphId.c_str()), atoi(partitionId.c_str()));
    graphDBMapCsvStores.insert(std::make_pair(graphIdentifier, jasmineGraphCsvStore));
    auto sg = graphDBMapCsvStores.find(graphIdentifier);
    instance_logger.log("###INSTANCE### Loading Csv Store : Completed", "info");
    return jasmineGraphCsvStore;
}
...
void JasmineGraphInstanceService::initStreamingServer(string partitionID){
    Utils utils;

    std::string path = "cd " + utils.getJasmineGraphProperty("org.jasminegraph.streaming.dl.location") + " && ";
    std::string command = path + "nohup python3 dl_server.py "+
            "--dataset_name " + utils.getJasmineGraphProperty("org.jasminegraph.csvstore.dataset.name") +
            " --partition_algorithm " + utils.getJasmineGraphProperty("org.jasminegraph.streaming.dl.partition_algorithm") +
            " --partition_size " + utils.getJasmineGraphProperty("org.jasminegraph.server.streaming.npartitions") +
            " --num_clients " + utils.getJasmineGraphProperty("org.jasminegraph.server.streaming.npartitions") +
            " --port " + utils.getJasmineGraphProperty("org.jasminegraph.streaming.dl.org.port") +
            " --num_of_timestamps " + utils.getJasmineGraphProperty("org.jasminegraph.csvstore.dataset.num_of_timestamps") +
            "  > wikipedia_server_2.log 2>&1 &";

    popen(command.c_str(), "r");
}

void JasmineGraphInstanceService::initStreamingClient(string partitionID){
    Utils utils;
    cout << "Partition ID: " + partitionID << endl;
    std::string path = "cd " + utils.getJasmineGraphProperty("org.jasminegraph.streaming.dl.location") + " && ";
    std::string command = path + "nohup python3 dl_client.py "+
            "--dataset_name " + utils.getJasmineGraphProperty("org.jasminegraph.csvstore.dataset.name") +
            " --partition_algorithm " + utils.getJasmineGraphProperty("org.jasminegraph.streaming.dl.partition_algorithm") +
            " --partition_size " + utils.getJasmineGraphProperty("org.jasminegraph.server.streaming.npartitions") +
            " --partition_id " + partitionID +
            " --port " + utils.getJasmineGraphProperty("org.jasminegraph.streaming.dl.org.port") +
            "  > wikipedia_client_2_"+partitionID+".log 2>&1 &";
