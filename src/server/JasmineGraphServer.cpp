...
void JasmineGraphServer::initiateStreamingDistributedLearning(SQLiteDBInterface sqlite) {

    Utils utils;
    int dl_clients = stoi(utils.getJasmineGraphProperty("org.jasminegraph.server.streaming.npartitions")) ;
    int threadLimit = dl_clients + 1;
    std::thread *workerThreads = new std::thread[threadLimit];
    ...
    for (int i = 0; i < workerVector.size(); i++) {
        ...
        if (i==0) {
            ...
            workerThreads[threadID] = std::thread(initiateStreamingServer,"localhost", serverPort,
                                                  serverDataPort, to_string(i));
            threadID++;
        }

        workerThreads[threadID] = std::thread(initiateStreamingClient,"localhost", serverPort, serverDataPort, to_string(i));
        threadID++;
    ...
    server_logger.log("Streaming distributed learning commands sent", "info");
...
bool JasmineGraphServer::initiateStreamingServer(std::string host, int port, int dataPort, string partitionID) {
    ...
    if (response.compare(JasmineGraphInstanceProtocol::HANDSHAKE_OK) == 0) {
        ...
        write(sockfd, JasmineGraphInstanceProtocol::INITIATE_STREAMING_SERVER.c_str(),
              JasmineGraphInstanceProtocol::INITIATE_STREAMING_SERVER.size());
        server_logger.log("Sent fed : " + JasmineGraphInstanceProtocol::INITIATE_STREAMING_SERVER, "info");
        bzero(data, FED_DATA_LENGTH);
        read(sockfd, data, FED_DATA_LENGTH);
        response = (data);
        response = utils.trim_copy(response, " \f\n\r\t\v");
//        if (response.compare(JasmineGraphInstanceProtocol::HOST_OK) == 0) {
        if (true) {
            ...
            server_logger.log("Sent fed : training args, worker id : " + partitionID, "info");
            bzero(data, FED_DATA_LENGTH);
            return 0;
        }
    } else {
...
bool JasmineGraphServer::initiateStreamingClient(std::string host, int port, int dataPort, string partitionID) {
    Utils utils;
    bool result = true;
    int sockfd;
    char data[FED_DATA_LENGTH];
    ...
    if (response.compare(JasmineGraphInstanceProtocol::HANDSHAKE_OK) == 0) {
        ...
        write(sockfd, JasmineGraphInstanceProtocol::INITIATE_STREAMING_CLIENT.c_str(),
              JasmineGraphInstanceProtocol::INITIATE_STREAMING_CLIENT.size());
        server_logger.log("Sent fed : " + JasmineGraphInstanceProtocol::INITIATE_STREAMING_CLIENT, "info");
        bzero(data, FED_DATA_LENGTH);
        read(sockfd, data, FED_DATA_LENGTH);
        response = (data);
        response = utils.trim_copy(response, " \f\n\r\t\v");
